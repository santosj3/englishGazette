...
const IndexPage = ({ data }) => (
  <>
    ...
    <Helmet title="Gazeta Esportiva" titleTemplate={"%s"}>
      ...
      <meta property="og:description" content={`Tudo sobre desporto motorizado`} />
      <meta property="og:image" content={Logo} />
      <meta name="twitter:card" content="summary_large_image" />
      <meta name="twitter:title" content="Gazeta Esportiva" />
      <meta name="twitter:description" content={`Tudo sobre desporto motorizado`} />
      ...
      {data?.allStrapiArticle?.edges.slice(0, 3).map((article) =>
        <div class="single-blog-post-slide bg-img background-overlay-5 item-contentor" style={{
          ...
          <div class="post-sozinho">
            ...
            <div class="date">
              <Link to={article.node.url} ><Date date={article.node.dateToPresent} datetime={article.node.datetime}></Date></Link>
            </div>
          </div>
        </div>
      )}
            ...
            <div class="gazette-todays-post ">
              ...
              {data?.allStrapiArticle?.edges.slice(3, 9).map((article) =>
                <div class="gazette-single-todays-post d-md-flex align-items-start mb-50">
                  <div class="image-single-post-hp" style={{ marginRight: "2%" }}>
                    <Image style={{ width: "100%", height: "100%" }} src={article.node.cover} alt={article.node.title} />
                  </div>
              ...
              <Link to={article.node.url} class="col-12 col-md-4">
                <div class="gazette-single-catagory-post">
                  ...
                  <>
                    ...
                    <span><Date date={article.node.dateToPresent} datetime={article.node.datetime}></Date></span>
                  </>
                </div>
              </Link>
            )}
